...
def test_select_with_classifier_for_fit_attribute(X, y) -> None:
...
def test_select_with_classifier_return_types(clf, X, y) -> None:
    result = select_with_classifier(clf, X, y)
    assert isinstance(result[0], pd.DataFrame)
    assert isinstance(result[1], pd.Series)


def test_select_with_classifier_shape_without_k_best(clf, X, y) -> None:
    result = select_with_classifier(clf, X, y)
